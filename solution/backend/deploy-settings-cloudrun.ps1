...
$SettingsRoot = Join-Path $ScriptDir "SettingsApi"
$SolutionRoot = Split-Path $ScriptDir -Parent                 # solution/
$DockerfilePath = Join-Path $SettingsRoot "Dockerfile"
$TempDockerfile = Join-Path $SolutionRoot "Dockerfile"        # copy to solution root
$ServiceAccountPath = Join-Path (Join-Path $ScriptDir "config") "serviceAccount.json"
...
if ([string]::IsNullOrWhiteSpace($ServiceName)) { $ServiceName = "magidesk-settings" }
