param(
    ...
    [Parameter(Mandatory=$false)][string]$ServiceName = "magidesk-discount",
...
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path  # solution/backend/DiscountApi
$SvcRoot = $ScriptDir
$SolutionRoot = Split-Path (Split-Path $ScriptDir -Parent) -Parent  # solution/
...
if ([string]::IsNullOrWhiteSpace($ServiceName)) { $ServiceName = "magidesk-discount" }
