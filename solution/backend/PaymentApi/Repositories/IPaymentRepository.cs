...
public interface IPaymentRepository
{
    ...
    Task InsertPaymentsAsync(NpgsqlConnection conn, NpgsqlTransaction tx, Guid sessionId, Guid billingId, Guid? cajaSessionId, string? serverId, IReadOnlyList<RegisterPaymentLineDto> lines, CancellationToken ct);
    ...
    // Refund methods
    Task<RefundDto> InsertRefundAsync(NpgsqlConnection conn, NpgsqlTransaction tx, Guid paymentId, Guid billingId, Guid sessionId, Guid? cajaSessionId, decimal refundAmount, string? refundReason, string refundMethod, string? externalRef, object? meta, string? createdBy, CancellationToken ct);
