param(
    ...
    [Parameter(Mandatory=$false)][string]$ServiceName = "magidesk-tables",
    [Parameter(Mandatory=$false)][string]$ConnString,
...
$TablesRoot = Join-Path $ScriptDir "TablesApi"
$SolutionRoot = Split-Path $ScriptDir -Parent                 # solution/
$DockerfilePath = Join-Path $TablesRoot "Dockerfile"
...
if ([string]::IsNullOrWhiteSpace($ServiceName)) { $ServiceName = "magidesk-tables" }
...
if ($ConnString -and $ConnString.Trim().Length -gt 0) {
    Write-Info "Including TABLESAPI__CONNSTRING env var"
    $envPairs += "TABLESAPI__CONNSTRING=$ConnString"
}
