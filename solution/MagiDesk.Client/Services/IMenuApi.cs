...
public interface IMenuApi
{
    ...
    Task<IApiResponse<MenuItemDto>> GetItemAsync(long id, CancellationToken ct = default);

    [Post("/api/menu/items")]
    Task<IApiResponse<MenuItemDto>> CreateMenuItemAsync([Body] CreateMenuItemDto dto, CancellationToken ct = default);

    [Put("/api/menu/items/{id}")]
    Task<IApiResponse<MenuItemDto>> UpdateMenuItemAsync(long id, [Body] UpdateMenuItemDto dto, CancellationToken ct = default);

    [Delete("/api/menu/items/{id}")]
    Task<IApiResponse<object>> DeleteMenuItemAsync(long id, CancellationToken ct = default);
}
